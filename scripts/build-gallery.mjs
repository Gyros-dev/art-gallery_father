...
const CATEGORY_LABELS = { Enamel: 'Горячая эмаль', Object: 'Арт-объект' };
// Порядок категорий (кто не указан — по алфавиту после указанных).
const CATEGORY_ORDER = ['Горячая эмаль', 'Арт-объект', 'Enamel', 'Object'];
...
const PAGES = [
  ...
  { file: 'pages/gallery.html',    loc: 'pages/gallery.html',     priority: '0.9',
    ...
    desc:  () => 'Произведения живописной горячей эмали Ивана Дьякова: работы по меди с описанием техники, размеров и года создания.' },
  { file: 'pages/exhibitions.html', loc: 'pages/exhibitions.html', priority: '0.8',
    title: (s) => `Выставки — ${s.name}`,
    desc:  () => 'Выставки Ивана Дьякова: текущие и будущие экспозиции, архив персональных и совместных проектов в России и за рубежом.' },
  { file: 'pages/about.html',      loc: 'pages/about.html',       priority: '0.8',
    title: (s) => `Художник — ${s.name}`,
    desc:  () => 'Иван Дьяков — художник-эмальер, член Союза художников России, доцент СПбГУ, награждён Золотой медалью Российской академии художеств. Биография, регалии, выставки.' },
  { file: 'pages/literature.html', loc: 'pages/literature.html',  priority: '0.6',
    title: (s) => `Литература — ${s.name}`,
    desc:  () => 'Публикации о творчестве Ивана Дьякова, материалы о технике горячей эмали и источники о работах художника.' },
];
    ...
    '@graph': [
      {
        ...
        name: site.name, jobTitle: 'Художник-эмальер',
        description: site.description,
        url: `${base}/`,
        address: {
          '@type': 'PostalAddress',
          addressLocality: String(site.location || '').split(',')[0].trim() || 'Санкт-Петербург',
          addressCountry: 'RU',
        },
        knowsAbout: ['Горячая эмаль', 'Живописная эмаль', 'Монументальное искусство', 'Декоративно-прикладное искусство'],
      },
      {
...
/** Страница 404 отдаётся по любому адресу, поэтому относительные пути на ней
    работать не могут: нужен путь от корня сайта. А корень разный — свой домен
    или подпапка вида /art-gallery_father на GitHub Pages. Берём его из
    data/site.json → url и проставляем во все ссылки с data-abs. */
async function fix404Paths(site) {
  const file = path.join(ROOT, '404.html');
  let html;
  try { html = await readFile(file, 'utf8'); } catch { return; }
  let prefix = '';
  try { prefix = new URL(site.url).pathname.replace(/\/+$/, ''); } catch { /* адрес не разобрался */ }
  // Стили тут тоже получают метку версии: обычный versionAssets работает
  // только с относительными путями, а на этой странице они от корня сайта.
  const stamp = async (target) => {
    if (!/\.(css|js)$/.test(target)) return '';
    try {
      const buf = await readFile(path.join(ROOT, target));
      return `?v=${createHash('md5').update(buf).digest('hex').slice(0, 8)}`;
    } catch { return ''; }
  };
  const parts = [];
  const re = /(href=")[^"]*("\s+data-abs="([^"]*)")/g;
  let last = 0, m;
  while ((m = re.exec(html)) !== null) {
    parts.push(html.slice(last, m.index), `${m[1]}${prefix}/${m[3]}${await stamp(m[3])}${m[2]}`);
    last = m.index + m[0].length;
  }
  parts.push(html.slice(last));
  const out = parts.join('');
  if (out !== html) await writeFile(file, out);
  console.log(`Страница 404 привязана к корню сайта: ${prefix || '/'}`);
}
